...
<div class="rounded-3xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-md transition">
    ...
    <div class="p-5">
        ...
        {{-- Actions --}}
        @if($isReviewOnly)
            {{-- pending/review: gak ada tombol sama sekali, card klik aja --}}
            <a href="{{ $detailUrl }}"
               class="mt-5 inline-flex w-full justify-center rounded-full bg-amber-50 px-5 py-2.5 text-sm font-semibold text-amber-700 border border-amber-200 hover:bg-amber-100 transition">
                Lihat Detail Review
            </a>
        @else
