            ...
            <div class="grid md:grid-cols-[3fr,2fr] gap-8 items-center">
                <div>
                    ...
                    <h1 class="text-3xl sm:text-4xl font-semibold leading-tight">
                        Wujudkan
                        <span class="underline decoration-lime-300 decoration-[4px] underline-offset-4">dana punia</span><br>
    ...
    <section id="daftar-program" class="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        <form method="GET" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 mb-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                {{-- Kategori (NOTE: ini masih UI saja; biar benar2 filter perlu kolom subkategori) --}}
                <div class="relative">
        ...
        @if (!isset($programs) || $programs->isEmpty())
            <div class="bg-white border border-dashed border-slate-200 rounded-2xl p-8 sm:p-10 text-center">
                <div class="mx-auto h-12 w-12 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center mb-4">
        ...
        @else
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 mt-6">
                @foreach ($programs as $program)
                    @php
                        $raised = (int) $program->raised; // accessor Program::getRaisedAttribute()
                    ...
                    <a href="{{ route('programs.show', $program->slug) }}"
                       class="group block bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition overflow-hidden">
                        ...
                        <div class="p-3 space-y-2">
                            ...
                            {{-- Kategori --}}
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px]
                            ...
                            {{-- Judul --}}
                            <h3 class="text-[13px] font-semibold text-slate-900 leading-snug line-clamp-2 min-h-[2.4rem]">
                                {{ $program->title }}
                            </h3>
                            ...
                            <div class="mt-1">
                                ...
                                <div class="text-[12px] font-semibold text-slate-900 flex justify-between">
                                    ...
                                    <span>
                                        {{ $isUnlimited ? 'Tanpa target' : 'Rp ' . number_format($target, 0, ',', '.') }}
                                    </span>
                                </div>
                            </div>
                            ...
                            {{-- CTA --}}
                            <div class="pt-1">
                                <span class="inline-flex items-center gap-1 text-[12px] font-semibold text-emerald-700">
            ...
            <style>
                @keyframes indet-1 {
                    0% { transform: translateX(-120%) scaleX(0.35); opacity: 0.9; }
                    60% { transform: translateX(180%) scaleX(0.85); opacity: 0.9; }
                    100% { transform: translateX(220%) scaleX(0.55); opacity: 0; }
                }
                @keyframes indet-2 {
                    0% { transform: translateX(-140%) scaleX(0.25); opacity: 0.6; }
                    40% { transform: translateX(120%) scaleX(0.65); opacity: 0.6; }
                    100% { transform: translateX(240%) scaleX(0.45); opacity: 0; }
                }
                .punia-indet {
                    position: relative;
                    height: 8px;
